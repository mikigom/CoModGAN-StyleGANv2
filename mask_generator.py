...
class RandomMaskGenerator:
    def __init__(self, batch_size, s, device, hole_range=[0, 1]):
    ...
    def __call__(self, batch_size=None):
        if batch_size is None:
            mask_np = BatchRandomMask(self.batch_size, self.s, self.hole_range)
        else:
            mask_np = BatchRandomMask(batch_size, self.s, self.hole_range)
        return torch.FloatTensor(mask_np).to(self.device)
